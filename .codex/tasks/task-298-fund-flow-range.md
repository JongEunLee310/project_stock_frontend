...
개요 화면의 `예상 자금 흐름` 패널이 모든 섹터를 `범위 미제공`으로 표시한다. 계약 타입과
어댑터만 고쳐 값이 다시 나오게 한다.

## Goal

- 개요 화면의 `예상 자금 흐름` 패널이 섹터마다 실제 구간을 표시한다.
- 구간이 없으면(`null`) 지어내지 않고 `범위 미제공`을 그대로 유지한다.
- **`src/widgets/`와 `src/pages/` 아래 파일이 한 줄도 바뀌지 않는다.** 아래 Protected Files 참조.
...
프론트엔드 표현은 현재 사용자가 직접 다듬는 중이며 **동결 상태다.** 이 결함은 표현 문제가
아니라 데이터 연결 문제이므로, 고칠 자리는 `dto.ts`와 `adapters.ts`뿐이다.

**뷰 필드 이름 `estimatedRange`와 타입 `string | null`을 그대로 유지하면** 위젯이 참조하는
코드가 바뀌지 않는다. 위젯·페이지 테스트의 픽스처도 뷰 레벨 문자열을 담고 있어 무수정으로
통과한다. 바뀌는 것은 그 문자열을 **어디서 만들어 오는지**뿐이다.

원시 수치(`low`·`high`)를 뷰에 남기지 않으므로, 이후 막대 길이를 값에 비례시킬 때(#294) 다시
파싱해야 한다. 그건 #294가 감당할 몫이고 지금 미리 만들지 않는다.

## Implementation Scope

- `src/features/news-insights/dto.ts` — `estimated_range`를 `estimated_flow`로 교체한다.
  구간 모양은 이름 붙은 타입으로 뽑는다(시나리오의 `expected_net_flow`가 같은 모양이다).
- `src/features/news-insights/adapters.ts` — 뷰 타입 `FundFlowOutlookItemView`의
  **`estimatedRange: string | null`은 그대로 둔다.** 값을 `estimated_flow`에서 만들어 넣는
  헬퍼를 더한다. 헬퍼는 표시 문자열 또는 `null`을 반환한다.
- `src/features/news-insights/adapters.test.ts`·`queries.test.tsx` — 와이어 레벨 픽스처를 새
  계약으로 옮기고 경계값 테스트를 더한다.
...
`src/widgets/**`와 `src/pages/**` 전체 — 소스와 테스트 모두.

이 브랜치는 `origin/main`에서 갓 분기했으므로 두 경로에 아직 아무 변경도 없다. 작업이 끝난
뒤에도 없어야 한다. 이 경로를 고쳐야 할 것 같으면 **고치지 말고 보고한다.** 그 필요가
생겼다는 것은 뷰 필드 이름·타입을 유지하지 못했다는 뜻이므로, 접근을 되짚어야 한다.

## Requirements

- 구간이 `null`이거나 `low`·`high`를 유한한 수로 읽을 수 없으면 `null`을 반환한다. **대체
  숫자를 만들지 않는다.** `parseDecimal`은 `Number()` 기반이라 `'Infinity'`가 통과하므로
  `Number.isFinite` 가드가 필요하다.
...
- 픽스처를 **실제 시드값**으로 맞춘다. BE `app/domains/news_insights/seed.py`의 반도체 항목이
  ...
  값을 쓰지 않는다. 이 값의 표시 문자열은 `8,000~18,000억원`이 된다.
- 표시 문자열 규칙을 값별로 덮는다 — 억 이상, 억 미만, 음수를 포함한 구간, 비-KRW 통화,
  `low === high`, `null`, 읽을 수 없는 값.
- **위젯·페이지 테스트가 무수정으로 통과하는지 확인한다.** 통과하지 않으면 그 자체가 이 방식이
  성립하지 않는다는 신호이므로, 고치지 말고 보고한다.
- 기존 단언을 약화시키지 않는다.
...
추가로 아래를 실행해 보호 경로가 정말 무변경인지 확인하고 출력을 그대로 보고한다.

```
git diff --stat origin/main...HEAD -- src/widgets/ src/pages/
```
...
통과한 채 조용히 빈 값을 표시했다.** 구현 중 같은 구조(응답에 없는 필드를 선언해 둔 곳)를 더
발견하면 고치지 말고 **목록으로 보고한다.**

## Risk Level

Low — 두 파일의 타입·매핑 교체다. 주의할 곳은 표시 문자열 규칙의 경계값(억 미만·음수·동일값)과
보호 경로를 건드리지 않는 것이다.
...
- 검증 5종 결과 보고.
- `git diff --stat origin/main...HEAD -- src/widgets/ src/pages/` 출력 그대로 보고.
...
- 현재 체크아웃된 브랜치(fix/298-fund-flow-range)를 유지한다(자체 브랜치 생성·push·PR 금지).
