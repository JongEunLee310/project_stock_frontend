...
이 기록은 **통제 실험이 아니라 단일 세션 관찰**이다. 아래 교란변수로 "Opus vs Sonnet+VFF"를 깨끗이 분리할 수 없다.

1. **작업 비동질**: BE#97(파이썬 API 신설) vs FE#47(TS 도메인 재정렬) — 도메인·복잡도·범위 상이.
2. **역할 비대칭**: BE=Opus 전담, FE=Opus 오케스트레이터+Sonnet 설계·리뷰 하이브리드(순수 대조 아님).
3. **측정 격리 실패**: Opus 토큰은 한 세션에 양 트랙+메타+디버깅이 누적돼 분리 불가. cache_read(비용 절반)는 작업이 아니라 **세션 길이**에 종속.
4. **순서·학습 효과**: BE 선행 → 그 동결 계약·OQ 답변이 FE 설계를 도움(독립 시행 아님).
5. **회계 단위 불일치**: 서브에이전트 단일 합계(캐시 미분해) vs Opus 트랜스크립트(캐시 분해), 콜드스타트 vs 웜캐시.
6. **n=1·인적 개입 불균등**: 반복 없음, codex hang 디버깅·승인 분기가 한쪽 트랙에만 발생.

**제대로 된 통제 설계(차기 라운드 권고)**:

- 동질/매칭 작업을 **카운터밸런싱**(순서 교차)하거나 동일 작업을 2회.
- 각 암(arm)을 **새 세션(클린 컨텍스트)**에서 실행 → 토큰을 작업 단위로 격리, cache 베이스라인 리셋.
- 역할 분담을 **사전 고정**(거부 마찰 157k 낭비 제거).
- 지표를 **사전 정의**: 토큰(캐시 분해)·$·실시간·품질 루브릭(계약 적합·리뷰 적발 결함·범위 정확도)·인적 개입 수.
- 설계·리뷰 산출물을 **블라인드 채점**.
- **n≥3 반복**(소규모 동질 작업 무작위 배정)으로 분산 추정.

→ 따라서 본 문서의 수치는 **방향성 참고**로만 쓰고, 단정적 우열 결론은 내리지 않는다.
