...
const detail: AssetDetailDto = {
  ...
  target_price_high: '1300.00',
  target_price_low: '900.00',
...
describe('research adapters', () => {
  ...
  it('returns summary-derived fallbacks when no saved summary exists', () => {
    const view = adaptResearchDetail(detail, null, checklist, thesis)
    ...
    expect(view.counterPoints).toEqual([])
    expect(view.keyRisks).toEqual([])
    expect(view.buyChecklist).not.toEqual([])
  })
