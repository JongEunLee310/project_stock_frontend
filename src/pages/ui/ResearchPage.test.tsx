...
describe('ResearchPage', () => {
  ...
  it('renders stance details in the confidence tooltip and metric tiles', async () => {
    ...
    expect(
      screen.getByText(
        (_, element) =>
          element?.tagName === 'LI' &&
          element.textContent === '최저 $1,000.00· KGI Securities',
      ),
    ).toBeVisible()
    expect(
      screen.getByText(
        (_, element) =>
          element?.tagName === 'LI' &&
          element.textContent === '최고 $1,200.00· JPMorgan',
      ),
    ).toBeVisible()
  ...
  it('uses the consensus fallback when all analyst target prices are null', async () => {
    mockUseAnalystOpinions.mockReturnValue({
      data: [
        {
          firm: 'KGI Securities',
  ...
  it('renders the recent opinion range when consensus bounds are absent', async () => {
    mockUseAnalystOpinions.mockReturnValue({
      data: [
        {
          firm: 'Recent Securities',
          action: 'main',
          toGrade: 'Buy',
          fromGrade: null,
          priceTarget: 510,
    ...
    expect(
      screen.getByText(
        (_, element) =>
          element?.tagName === 'LI' &&
          element.textContent === '최저 $510.00· Recent Securities',
      ),
    ).toBeVisible()
    expect(
      screen.getByText(
        (_, element) =>
          element?.tagName === 'LI' &&
          element.textContent === '최고 $510.00· Recent Securities',
      ),
    ).toBeVisible()
